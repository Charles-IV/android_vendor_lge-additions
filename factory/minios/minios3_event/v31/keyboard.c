...
struct {
    ...
    Event_t evt;    /* the event we are supposed to repeat */
} KeyRepeat;

/* Global no-lock-keys support */
static Uint8 NoLockKeys;

#define NLK_CAPS 0x01
#define NLK_NUM  0x02
...
int KeyboardInit(void)
{
    ...
    EnableUNICODE(DEFAULT_UNICODE_TRANSLATION);
    ...
    NoLockKeys = 0;
    env = getenv("DISABLE_LOCK_KEYS");
    if (env) {
        switch (atoi(env)) {
            case 1:
                NoLockKeys = NLK_CAPS | NLK_NUM;
                break;
            case 2:
                NoLockKeys = NLK_CAPS;
                break;
    ...
    keynames[SDLK_HOOK] = "hook";
    keynames[SDLK_RECENT] = "recent_app";
    keynames[SDLK_DSIM] = "dual_sim";
    keynames[SDLK_QMEMO] = "quick_memo";
    keynames[SDLK_ENTER] = "ok";
    keynames[SDLK_MESSAGE] = "message";
    keynames[SDLK_PHONEBOOK] = "phonebook";
    keynames[SDLK_LANG] = "lang";
    keynames[SDLK_SEND] = "send";
    keynames[SDLK_VOLUMEUP] = "vol_up";
    keynames[SDLK_VOLUMEDOWN] = "vol_down";
    keynames[SDLK_KAKAO] = "kakao";
    keynames[SDLK_CAMERA] = "camera";
    keynames[SDLK_CLR] = "clr";
    keynames[SDLK_HOT] = "hot_key";
    /* Done.  Whew. */
    return(0);
}
...
int EnableUNICODE(int enable)
{
    ...
    old_mode = TranslateUNICODE;
    if ( enable >= 0 ) {
        TranslateUNICODE = enable;
    }
    return(old_mode);
}
...
int GetKeyCode(char *name)
{
    ...
    for (index = 0; index < SDLK_LAST; index++) {
        if(keynames[index] != NULL) {
            if(strcmp(name, keynames[index]) == 0) {
                return index;
            }
        }
    }
...
int PrivateKeyboard(Uint8 state, keysym *keysym)
{
    Event_t event;
...
#if 0
printf("The '%s' key has been %s\n", GetKeyName(keysym->sym),
                state == PRESSED ? "pressed" : "released");
#endif
    /* Set up the keysym */
    modstate = (Uint16)ModState;
    ...
    switch (state) {
        ...
        case RELEASED:
             ...
             * jk 991215 - Added
             */
            if ( KeyRepeat.timestamp &&
                 KeyRepeat.evt.key.keysym.sym == keysym->sym ) {
                KeyRepeat.timestamp = 0;
            }
            break;
        default:
...
printf("Keyboard event didn't change state - dropped!\n");
    ...
    if ( ProcessEvents[event.type] == ENABLE ) {
         ...
         * jk 991215 - Added
         */
        if (repeatable && (KeyRepeat.delay != 0)) {
            KeyRepeat.evt = event;
            KeyRepeat.firsttime = 1;
            KeyRepeat.timestamp=GetTicks();
        }
...
void CheckKeyRepeat(void)
{
    if ( KeyRepeat.timestamp ) {
        ...
        now = GetTicks();
        interval = (now - KeyRepeat.timestamp);
        if ( KeyRepeat.firsttime ) {
            if ( interval > (Uint32)KeyRepeat.delay ) {
                KeyRepeat.timestamp = now;
                KeyRepeat.firsttime = 0;
            }
        } else {
            if ( interval > (Uint32)KeyRepeat.interval ) {
                KeyRepeat.timestamp = now;
                if ( (EventOK == NULL) || EventOK(&KeyRepeat.evt) ) {
                    PushEvent(&KeyRepeat.evt);
                }
            }
        }
    }
...
int EnableKeyRepeat(int delay, int interval)
{
    if ( (delay < 0) || (interval < 0) ) {
        SetError("keyboard repeat value less than zero");
        return(-1);
    }
    KeyRepeat.firsttime = 0;
...
{
    *delay = KeyRepeat.delay;
    *interval = KeyRepeat.interval;
}
