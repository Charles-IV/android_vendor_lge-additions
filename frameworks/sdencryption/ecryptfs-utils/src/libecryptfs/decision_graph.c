...
int stack_push(struct val_node **head, void *val)
{
    ...
    node->val = val;
    node->next = *head;
    syslog(LOG_INFO, "%s:%d:: hyunk node  val=%s\n", __FUNCTION__,__LINE__, node->val);
    *head = node;
out:
    return rc;
}
        ...
        if (tn->val && current->val
            ...
            if (tn->trans_func) {
                rc = tn->trans_func(ctx, current,
                            mnt_params, foo);
                syslog(LOG_INFO, "%s, %d, rc=[%d]\n", __FUNCTION__, __LINE__, rc); //sw.yoo_20120112 - print debug
            }
            ...
            if (strcmp(temp->name, node->mnt_opt_names[i]) == 0
                ...
                if (temp->value
                    ...
                    if (asprintf(&node->val, "%s",
                             temp->value) == -1) {
                        rc = -ENOMEM;
            syslog(LOG_INFO, "%s:%d:: hyunk Value retrieval "
...
#if 0 // hyunk
        if (!strcmp(node->mnt_opt_names[0], "key"))
        {
            free(node->val);
            node->val = NULL;
        }
    ...
    do {
        if (ecryptfs_verbosity) {
            ...
            syslog(LOG_INFO, "%s: Calling alloc_and_get_val() on "
                   "node = [%p]; node->mnt_opt_names[0] = [%s], val = [%s]\n",
                   __FUNCTION__, node, node->mnt_opt_names[0], node->val?node->val:"null");
            for (i = 0; i < node->num_transitions; i++) {
                syslog(LOG_INFO,
                       "%s:  node->tl[%d].val = [%s]\n",
...
//    ecryptfs_dump_decision_graph (NULL, root_node, 0); // hyunk DEBUG
                   ...
                   int depth, int recursive)
{
    ...
    syslog(file_stream, "---------------\n");
    print_whitespace(file_stream, depth);
    syslog(file_stream, "transition_node\n");
    print_whitespace(file_stream, depth);
    syslog(file_stream, "---------------\n");
    print_whitespace(file_stream, depth);
    syslog(file_stream, "val = [%s]\n", trans_node->val);
    print_whitespace(file_stream, depth);
    syslog(file_stream, "next_token = [%p]\n", trans_node->next_token);
                  ...
                  int recursive)
{
    ...
    syslog(file_stream, "----------\n");
    print_whitespace(file_stream, depth);
    syslog(file_stream, "param_node\n");
    print_whitespace(file_stream, depth);
    syslog(file_stream, "----------\n");
    print_whitespace(file_stream, depth);
    syslog(file_stream, "mnt_opt_names[0] = [%s]\n",
        param_node->mnt_opt_names[0]);
    print_whitespace(file_stream, depth);
    syslog(file_stream, "num_transitions = [%d]\n",
        param_node->num_transitions);
    for (i = 0; i < param_node->num_transitions; i++) {
        print_whitespace(file_stream, depth);
        syslog(file_stream, "transition node [%d]:\n", i);
